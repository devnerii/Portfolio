          ...
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex items-center space-x-4">
              <Link href="/" className="flex items-center space-x-2">
                ...
                >
                  <LogoSVG className="w-full h-auto max-w-6xl sm:w-full sm:h-auto lg:w-full lg:h-auto" />
                </motion.div>
              </Link>
            </div>
            ...
            <div className="hidden md:flex items-center space-x-8">
              {menuItems.map((item) => (
                <button
                  ...
                  className="text-sm sm:text-base md:text-lg lg:text-lg xl:text-xl 2xl:text-xl font-medium text-white dark:text-white hover:text-green-400 transition duration-300 dark:hover:text-blue-400"
                >
            ...
            <div className="flex items-center space-x-4 relative">
              ...
              <div className="relative hidden md:block">
                <motion.button
                  className="flex items-center space-x-2 p-2 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-700"
        ...
        >
          {menuItems.map((item) => (
            <button
              key={item.id}
              onClick={() => handleSmoothScroll(item.id)}
              className="block py-3 px-6 text-black hover:bg-green-500 dark:text-white dark:hover:bg-blue-700 hover:text-white transition duration-300 rounded-lg"
            >
              {capitalize(t(item.label))}
            </button>
          ))}
          ...
          <div className="flex flex-col items-start px-6 py-3">
            <button
              onClick={() => changeLanguage('pt')}
              className="flex items-center space-x-2 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 w-full rounded-lg"
            >
              <Flag code="BRA" className="w-5 h-5" alt="Portuguese" />
              <span className="text-black dark:text-white">Português</span>
            </button>
